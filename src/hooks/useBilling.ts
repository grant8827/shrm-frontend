...
export function useBilling() {
  ...
  const createInvoice = useCallback(
    async (data: Partial<Invoice> & { total: number; patientId: string }) => {
      const res = await apiClient.post('/api/billing/invoices', data);
  ...
  const updateInvoiceStatus = useCallback(
    async (id: string, patch: Partial<Invoice>) => {
      ...
      try {
        const res = await apiClient.patch(`/api/billing/invoices/${id}`, patch);
  ...
  const addPayment = useCallback(
    async (id: string, amount: number, paymentMethod?: string) => {
      ...
      try {
        const res = await apiClient.post(`/api/billing/invoices/${id}/payment`, {
          amount,
          paymentMethod,
        });
        const updated = res.data as Invoice;
